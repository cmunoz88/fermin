...
(function () {
    ...
    function getCodeFromUrl(url, convert) {
        $.ajax({
            url: url,
            type: 'GET',
            success: function (res) {
                try {
                    init(JSON.stringify(convert(res), null, indentUnit));
                } catch (err) {
                    console.error(err);
                    console.warn('Silently ignoring this error...');
                    init();
                }
            },
    ...
    var converters = {
        ...
        XML: function (xml) {
            return window.xml2obj(xml);
        },
        CSV: function (csv) {
            return window.csv2obj(csv);
        },
        JSON: function (json) {
    ...
    var queryObj = decodeQuery();

    var importCode = queryObj.importCode || queryObj.importCodeJSON;
    var importUrl = queryObj.importUrl || queryObj.importUrlJSON;
    if (importCode) {
        try {
            init(JSON.stringify(converters.JSON(importCode)), null, indentUnit);
        } catch (err) {
            console.error(err);
            console.warn('Silently ignoring this error...');
            init();
        ...
        getCodeFromUrl(importUrl, converters.JSON);
    } else {
        var formats = ['YAML', 'XML', 'CSV'];
        (function () {
            for (var i = 0; i < formats.length; i++) {
                var format = formats[i];
                var converter = converters[format];
                importCode = queryObj['importCode' + format];
                importUrl = queryObj['importUrl' + format];
                if (importCode) {
                    init(JSON.stringify(converter(importCode), null, indentUnit));
                    return;
                } else if (importUrl) {
                    getCodeFromUrl(importUrl, converter);
                    return;
                }
            }
            init();
    ...
    function init(persistentValStr) {

        console.log(persistentValStr);

        persistentValStr = persistentValStr || localStorage.getItem('persistentVal') || JSON.stringify({
            "titulares":[
                {
                  "tipoActuacion":"C",
                  "calidadActua":"datos",
                  ...
                  "cliente":{
                    ...
                    "apellidoCasada":"datos",
                    "primerNombre":"datos",
                    "segundoNombre":"datos",
                    "otrosNombres":"datos",
                    "fechaNacimiento":"2020023",
                    "nacionalidades":[
                      "GT",
                      "MX",
                      "US"
                    ...
                    "condicionMigratoria":7,
                    "otraCondicionMigratoria":"datos",
                    "sexo":"M",
                    "estadoCivil":"S",
                    "profesionOficio":"",
                    "tipoDocumentoIdentificacion":"P",
                    "numeroDocumentoIdentificacion":"datos",
                    "emisionPasaporte":"MX",
                    "nit":"datos",
                    "telefonos":[
                      22000000,
                    ...
                    "datosPep":{
                      ...
                      "otroOrigenRiqueza":"datos"
                    },
                    "datosParientePep":[
                      {
                        "parentesco":"P",
                        "otroParentesco":"datos",
                        "complementoPAPEP":{
                          "primerApellido":"datos",
                          "segundoApellido":"datos",
                          "apellidoCasada":"datos",
              ...
              "productos":[
                {
                  ...
                  "fecha":"20200223",
                  "tipo":"datos",
                  "nombreDescripcion":"dastos",
                  "identificador":"datos",
                  "nombreContrata":"datos",
                  "moneda":"GTQ",
                  "valor":99.99,
                  "fondosPropios":"1",
                  "fondosTercero":"PI",
                  "personaIndividualProveeFondos":[
                    {
                      "primerNombre":"datos",
                      "segundoNombre":"datos",
                      "otrosNombres":"datos",
                      ...
                      "nit":"datos",
                      "sexo":"F",
                      "fechaNacimiento":"20220131",
                      "direccionResidencia":"datos",
                      "residencia":{
                        "pais":"GT",
                        "departamento":"01",
                        "municipio":"0101"
                  ...
                  "personaJuridicaProveeFondos":[
                    {
                      "nombreRazonSocial":"datos",
                      "datosPersonaJuridicaProveeFondos":{
                        "nombreComercial":"datos",
                        "paisJuridiccionConstitucion":"GT",
                        "nit":"",
                        "direccion":"",
                        "lugar":{
                          "pais":"GT",
...
<input type="checkbox" name="merge"> Merge patches</label>', false, function (result) {
                    if (result !== false) {
                        ...
                        setTimeout(function () {
                            $('html, body').animate({ scrollTop: $jsonPatchContainer.offset().top }, '100');
                        }, 0);
                    }
                });
        });
...
<option>POST</option>\
        ...
        $('#nav-file-new').on('click', function () {
            window.confirmModal('Are you sure 2?', 'You will lose all of your progress.', 'Ok', 'Cancel', function (result) {
                if (result) {
                    persistentVal = {};
